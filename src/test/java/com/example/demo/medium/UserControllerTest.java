...
class UserControllerTest {
    ...
    void 사용자는_특정_유저의_정보를_개인정보는_소거된_채_전달_받을_수_있다() throws Exception {
        ...
        mockMvc.perform(get("/api/users/1"))
                ...
                .andExpect(jsonPath("$.email").value("sylvan0212@gmail.com"))
                .andExpect(jsonPath("$.nickname").value("OR"))
                .andExpect(jsonPath("$.address").doesNotExist()) // 주소 항목 소거. 이런 항목이 모여 정책이 된다.
    ...
    void 사용자는_인증_코드로_계정을_활성화_시킬_수_있다() throws Exception {
        ...
        mockMvc.perform(get("/api/users/2/verify")
                        .queryParam("certificationCode", "aaaaaa-aaaaa-aaaa-aaab")
                )
    ...
    void 사용자는_인증_코드가_일치하지_않을_경우_권한없음_에러를_받는다() throws Exception {
        ...
        // then
        mockMvc.perform(get("/api/users/2/verify")
                        .queryParam("certificationCode", "aaaaaa-aaaaa-aaaa-aaac")
                )
    ...
    void 사용자는_내_정보를_불러올_때_개인정보인_주소도_갖고_올_수_있다() throws Exception {
        ...
        mockMvc.perform(get("/api/users/me")
                        .header("EMAIL", "sylvan0212@gmail.com")
                )
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.email").value("sylvan0212@gmail.com"))
    ...
    void 사용자는_내_정보를_수정할_수_있다() throws Exception {
        ...
        mockMvc.perform(put("/api/users/me")
                ...
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.email").value("sylvan0212@gmail.com"))
                .andExpect(jsonPath("$.nickname").value("OR5"))
